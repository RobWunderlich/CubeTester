...
const runTest = async function (test, index, argv) {
	let testResult = {};
	...
	try {
		session = getSession(test.target.endpoint, argv.certPath, argv.user, argv.domain, argv.trace);
		...
		//console.log('exit runTest ' + Date.now());
		return testResult;
